...
class TCN(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        #self.swinT = SwinT.SwinT(n_feats=in_channels)
        self.wavelet_attn = WaveletAttention(dim=in_channels, num_heads=3, window_size=8, num_blocks=1)

    def forward(self, x, H, W):
        # x shape: (B, C, H, W)
        x_out = self.wavelet_attn(x, H, W)
        #return x_out
...
class P_HTCB(nn.Module):
    ...
    def forward(self, x):
        ...
        h_tcn1 = self.tcn1(h_tesa, H, W)   # ✅ تمرير H, W
        h_tcn2 = self.tcn2(h_tesa, H, W)   # ✅ تمرير H, W
