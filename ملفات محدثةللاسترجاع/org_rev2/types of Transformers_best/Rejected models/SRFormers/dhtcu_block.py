...
from .custom_attention_blocks import PSA   # ✅ بدلاً من SwinT
...
class TCN(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.attn = PSA(dim=in_channels, window_size=4, num_heads=8)  # ✅

    def forward(self, x, H, W):
        # الـ x هنا هو خرج TESA (بأبعاد B, C, H, W)
        B, C, H, W = x.shape
        x_flat = x.flatten(2).transpose(1, 2)  # (B, H*W, C)
        x_attn = self.attn(x_flat, H, W)       # (B, H*W, C)
        x_attn = x_attn.transpose(1, 2).view(B, C, H, W)
        return x_attn
        #return self.conv3(x_attn)
