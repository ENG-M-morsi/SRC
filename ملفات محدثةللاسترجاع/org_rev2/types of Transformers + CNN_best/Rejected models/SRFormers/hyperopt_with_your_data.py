# ===================================================================
# hyperopt_with_your_data.py — بحث شامل مع PSA و HUTCN
# ===================================================================
import optuna
import torch
...
from model.custom_attention_blocks import PSA
...
def objective(trial):
    ...
    for module in model.modules():
        if isinstance(module, B.TCN):
            # ✅ التحقق من وجود attn و dim
            if hasattr(module, 'attn') and hasattr(module.attn, 'dim'):
                dim = module.attn.dim
            else:
                # إذا لم يكن هناك attn، نستخدم nf كقيمة افتراضية
                dim = nf
            
            # ✅ استبدال attn بـ PSA جديدة
            module.attn = PSA(
                dim=dim,
