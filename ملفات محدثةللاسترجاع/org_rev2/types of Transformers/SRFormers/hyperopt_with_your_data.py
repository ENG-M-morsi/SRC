# ===================================================================
# hyperopt_with_your_data.py — بحث شامل متوافق مع PSA و HUTCN
# ===================================================================
import optuna
import torch
...
from model.custom_attention_blocks import PSA   # استيراد PSA
...
# ===================================================================
# دالة إنشاء DataLoaders (نفس السابق مع تعديل بسيط)
# ===================================================================
def get_loaders_from_args(trial_params, base_args):
    # نسخ args وتعديل الخصائص المطلوبة
    args = base_args
...
def objective(trial):
    ...
    # تعديل معاملات PSA داخل النموذج
    for module in model.modules():
        if isinstance(module, B.TCN):
            # نستبدل الـ attn الحالية بـ PSA جديدة بالمعاملات المختارة
            dim = module.attn.dim   # نأخذ البعد من الـ attn الحالية
            module.attn = PSA(
                dim=dim,
