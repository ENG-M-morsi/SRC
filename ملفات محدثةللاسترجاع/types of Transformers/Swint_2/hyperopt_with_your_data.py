# ===================================================================
# hyperopt_with_your_data.py — بحث شامل متوافق مع SwinT و HUTCN
# ===================================================================
import optuna
import torch
...
from model.custom_attention_blocks import SwinT   # استيراد SwinT

# ===================================================================
# دوال الخسارة الإضافية (نفس السابق)
# ===================================================================
...
# ===================================================================
# دالة إنشاء DataLoaders (مع نسخ args لتجنب التعديل)
# ===================================================================
def get_loaders_from_args(trial_params, base_args):
    # نسخ args لتعديلها دون التأثير على الأصل
...
def objective(trial):
    ...
    # ---------- بناء النموذج ----------
    model = HUTCN(in_nc=3, nf=nf, num_modules=1, out_nc=3, upscale=4)

    # تعديل معاملات SwinT داخل النموذج
    for module in model.modules():
        if isinstance(module, B.TCN):
            # نستبدل الـ swint الحالية بـ SwinT جديدة بالمعاملات المختارة
            dim = module.swint.dim   # نأخذ البعد من الـ swint الحالية
            module.swint = SwinT(
                dim=dim,
                num_heads=num_heads,
            ...
            # ملاحظة: SwinT لا يقبل mlp_ratio حالياً (ثابت 2)،
            # ولكن يمكن تعديل الكود لاحقاً لقبوله.
    ...
    trial_params = {
        ...
        'batch_size': 8  # ثابت لتقليل التعقيد
    }
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_idx, (lr, hr, _) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            sr = model(lr)          # HUTCN يتوقع (lr) فقط
