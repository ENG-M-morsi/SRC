...
# (محسّن لتقليل استهلاك الذاكرة إلى أقصى حد)
# ===================================================================
import optuna
import torch
...
# ===================================================================
# دالة الهدف الرئيسية (مع تقليل الذاكرة إلى أقصى حد)
# ===================================================================
def objective(trial):
    # ---------- معاملات بنية النموذج (نطاقات ضيقة جداً) ----------
    nf = trial.suggest_int('n_feats', 16, 32, step=8)        # ← 16-32
    num_heads = 2  # ثابت لتقليل الذاكرة
    window_size = trial.suggest_categorical('window_size', [4, 6])  # ← 4 أو 6
    num_blocks = 1  # ثابت

    patch_size = trial.suggest_categorical('patch_size', [64, 80, 96])  # ← صغير
    batch_size = 2  # ثابت
    ...
    # ---------- جدول التوهين ----------
    EPOCHS = 3  # عدد قليل جداً للتجربة
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for batch_idx, (lr, hr, _) in enumerate(train_loader):
            ...
            loss.backward()
            optimizer.step()
            torch.cuda.empty_cache()  # تنظيف الذاكرة بعد كل دفعة
...
if __name__ == "__main__":
    ...
    print(f"🚀 بدء البحث الشامل ({N_TRIALS} محاولة، كل محاولة {3} Epochs)...")
