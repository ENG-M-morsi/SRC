...
from model.custom_attention_blocks import OmniSR  # ✅ استيراد OmniSR

# ===================================================================
# 1. دوال الخسارة الإضافية (Charbonnier & Huber)
# ===================================================================
...
# ===================================================================
# 3. دالة الهدف الرئيسية (Objective) — مع OmniSR
# ===================================================================
def objective(trial):
# ---------- (أ) معاملات بنية النموذج ----------
    ...
    # 2. عدد الرؤوس (num_heads) — يجب أن يقسم nf
    valid_heads = [h for h in [2, 4, 8, 16] if nf % h == 0]
    if not valid_heads:
        raise optuna.TrialPruned()
    num_heads = trial.suggest_categorical('num_heads', valid_heads)

    # 3. عدد كتل OmniSR (num_blocks) — يتحكم في عدد OSAG
    num_blocks = trial.suggest_int('num_blocks', 1, 4, step=1)

    # 4. حجم النافذة (window_size) — يجب أن يكون <= patch_size
    window_size = trial.suggest_categorical('window_size', [8, 12, 16])

    # 5. عدد كتل P_HTCB (ثابت = 1)
    num_modules = 1
    ...
    # ✅ إعادة تعريف OmniSR داخل كل TCN بالمعاملات الجديدة
