...
# Class of upscaling
###################################################
class UP_scal(nn.Module):
    def __init__(self, in_channels, upscale=3):
        super(UP_scal, self).__init__()
        self.recon_conv1     = B.conv_layer(in_channels, in_channels, kernel_size=3)   # Conv3×3 أولى
        self.recon_conv2     = B.conv_layer(in_channels, in_channels * (upscale**2), kernel_size=3)  # Conv3×3 ثانية
...
class TCN(nn.Module):
    ...
    def __init__(self, in_channels,  upscale=3):
        ...
        self.swinT = SwinT.SwinT(n_feats=in_channels)
        self.up_scal = UP_scal(in_channels,upscale = upscale)
        

    def forward(self, x):
        # HTCN = FSTL(FConv3(HTESA))
        #return self.swinT(self.conv3(x))
        return self.conv3(self.swinT(self.up_scal(x)))
...
class P_HTCB(nn.Module):
    ...
    def __init__(self, in_channels, upscale=3):
        ...
        self.tcn1 = TCN(in_channels, upscale = upscale)
        self.tcn2 = TCN(in_channels, upscale = upscale)
